...
const Card = ({ card, onClickLikeHandler }) => {
  const { city, street, house, room } = card.attributes.address;
  const { title, area, unit, rooms } = card.attributes;
  const { first_name, last_name, middle_name } = card.relationships.attributes;
  ...
  return (
    <div className="card">
      ...
      <div className="card__props">
        <span
          className={card.isLiked ? 'card__likes liked' : 'card__likes'}
          onClick={() => {
            onClickLikeHandler(card.id);
          }}
        >
      ...
      <div className="card__info">
        <span className="card__term">{`Адрес: ${city}, ${street}, ${house}, ${room},`}</span>
        <span className="card__term">{`Площадь: ${area} ${unit}`}</span>
        <span className="card__term">{`Кол-во комнат: ${rooms}`}</span>
      </div>
